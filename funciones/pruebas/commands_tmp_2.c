...
void commands(t_cmd *cmd, t_list *token)
{
	...
	// Find the first node with a token '<', '>', or '|'
	while (current != NULL && (current->content[0] != '<' && current->content[0] != '>' && current->content[0] != '|'))
	{
	...
	// Count the number of valid commands after the first node with '<', '>', or '|'
	while (current != NULL)
	{
		if (current->content[0] != '-')
			valid_cmds++;
	...
	int i = 0;
	current = token;
	// Skip nodes until the first node with '<', '>', or '|'
	while (current != NULL && (current->content[0] != '<' && current->content[0] != '>' && current->content[0] != '|'))
	{
		if (current->content[0] != '-')
		{
	...
	// Continue processing nodes after the first node with '<', '>', or '|'
	while (current != NULL)
	{
		if (current->content[0] != '-')
		{
