...
int main()
{
	...
	char *cmd_1[] = {"du", "-h", "../../../../", NULL};
	char *cmd_2[] = {"sort", "-rh", NULL};
	char *cmd_3[] = {"head", "-n", "5", NULL};
	...
	executeCommand(cmd_1, STDIN_FILENO, pipefd[1]);
	close(pipefd[1]);
	...
	executeCommand(cmd_2, pipefd[0], pipefd2[1]);
	close(pipefd[0]);
	close(pipefd2[1]);
	...
	executeCommand(cmd_3, pipefd2[0], filefd);
	close(pipefd2[0]);
	close(filefd);
