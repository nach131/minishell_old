...
void executePipeline(char ***commands, int num_commands, int output_fd)
{
	int pipefd[2];
	int prev_pipefd[2] = {STDIN_FILENO, output_fd};
...
int main()
{
	...
	char *cmd_cat[] = {"/bin/cat", "archivo.txt", NULL};
	char *cmd_grep[] = {"/usr/bin/grep", "42", NULL};
	char *cmd_sed1[] = {"/usr/bin/sed", "s/42/131/g", NULL};
	char *cmd_sed2[] = {"/usr/bin/sed", "s/Barcelona/New York/g", NULL};
	...
	executePipeline(commands, num_commands, filefd);
