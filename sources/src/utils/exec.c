...
/*   Updated: 2023/07/20 09:49:08 by nmota-bu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_builtin(t_data *data, t_cmd *cmd)
{
	...
	//
	if (cmd->num_cmd == 1)
		executeCommand(cmd->command[0], cmd->args[0], STDIN_FILENO,
	...
	else if (cmd->num_cmd == 2)
	{
		...
		else
		{
			executeCommand(cmd->command[0], cmd->args[0], STDIN_FILENO,
					cmd->filefd[0][OUT], cmd->env);
			close(cmd->filefd[0][OUT]);
			executeCommand(cmd->command[1], cmd->args[1], cmd->filefd[0][IN],
					STDOUT_FILENO, cmd->env);
	...
	else
	{
		...
		{
			// Si el último operador de redirección es '>',
				ejecutamos el último comando con el penúltimo pipe de entrada y el último pipe de salida
			executeCommand(cmd->command[i], cmd->args[i], cmd->filefd[i
					- 2][IN], cmd->filefd[i - 1][OUT], cmd->env);
			close(cmd->filefd[i - 1][OUT]);
		}
		// Si el último operador de redirección no es '>',
			ejecutamos el último comando con el penúltimo pipe de entrada y la salida estándar
		executeCommand(cmd->command[i], cmd->args[i], cmd->filefd[i - 1][IN],
				STDOUT_FILENO, cmd->env);
		close(cmd->filefd[i - 1][IN]);
